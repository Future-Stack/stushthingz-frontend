...
const BankLayout: React.FC = () => {
  ...
  const userName = user?.name || "Bank User";
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-gray-900 font-inter">
      {/* ── Top Navbar ───────────────────────────────────────────────────── */}
      <header className="w-full bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          {/* Logo */}
          <Link to="/bank" className="flex flex-col">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex justify-center items-center bg-gradient-to-br from-[#D91A7C] to-[#9c1654] rounded-full w-9 h-9 overflow-hidden shrink-0">
                      {profileImage ? (
                        ...
                        <span className="font-bold text-white text-xs">{initials || "B"}</span>
                      )}
                    </div>
