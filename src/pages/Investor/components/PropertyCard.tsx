...
import { useToggleWishlistMutation } from "../../../store/api/propertyApi";
...
const PropertyCard: React.FC<PropertyCardProps> = ({ property, isInterestPage = false }) => {
  ...
  const displayImage = property.propertyImages?.[0]?.url || "";
    ...
    <div className="bg-white border border-[#919EAB] rounded-2xl overflow-hidden flex flex-col lg:flex-row shadow-sm">
      ...
      <div className="w-full lg:w-[350px] xl:w-[400px] h-[240px] lg:h-auto relative shrink-0">
        ...
        <div className="absolute bottom-3 left-3 flex gap-2">
          {(property.statusTag || property.developmentStatus) && (
      ...
      <div className="p-5 lg:p-6 flex-1 flex flex-col">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6 bg-[#F9FAFB] rounded-[10px] p-4">
          <div>
            ...
            <p className="text-sm lg:text-base font-semibold text-color-main">
              {property.priceRangeLower} - {property.priceRangeUpper}
            </p>
          </div>
          <div>
            <p className="text-xs text-[#4A5565] font-normal mb-0.5">Size</p>
            <p className="text-sm lg:text-base font-semibold text-color-jet-black">{property.sizeArea}</p>
          </div>
          <div>
              ...
              <ul className="space-y-2 ml-1">
                {property.keyHighlights.map((item, idx) => (
                  <li key={idx} className="text-sm text-[#364153] font-normal items-end flex gap-2">
                    <span className="text-color-main font-bold text-[8px]">●</span> {item.content}
                  </li>
                ))}
              </ul>
            </div>
